...
def _(mo):
    ...
    Training a Dense Neural Network to classify<br>handwritten digits images from MINIST database</div>
    """)
    return
...
def _(mo):
    mo.md(r"""
    ## 1 - Load the MNIST data (images and labels)
    """)
    return
...
def _(mo):
    mo.md(r"""
    With the `plot_image` function (from the `utils.tools` module) we can display images and labels starting with image #600 on a grid of 4 x 12:
    """)
    return
...
def _(mo):
    mo.md(r"""
    Now let's verify that the proportion of digits remains homogenous through the new `valid` & `test` datasets:
    """)
    return
...
def _(NB_CLASS, NB_PIXEL, SEED, build_DNN, save_model):
    ...
    save_model(_model, f'dense1_init_{SEED}.keras', list_dir=True)
    return
...
def _(
    ...
    with mo.persistent_cache(name="DNN_cache"):
        ...
        model_init = tf.keras.models.load_model(Path('models') / f'dense1_init_{SEED}.keras') 
